...
NeNe Profile is a consumer application built on [NENE2](https://github.com/hideyukiMORI/NENE2). The maintainer wants the same strict engineering discipline as NENE2 and sibling NeNe products (Records, Corpus, Concierge) — Issue-driven workflow, Conventional Commits, self-review checklists, and AI-readable documentation — without copying the entire NENE2 documentation tree.

Alternatives considered:

1. **Link-only** — rejected; product-specific rules (CSV normalization, compliance, sibling boundaries) would be unclear.
...
- Product rules (CSV normalization model, upstream boundaries) stay separate from framework rules.
...
- Add CSV normalization and compliance ADRs as domains stabilize.
