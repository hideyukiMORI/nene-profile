...
/**
 * Parses `YYYY-MM-DD` or `YY-MM-DD` to ISO 8601. Two-digit years use the
 * context pivot (ADR 0003 §2).
 */
final readonly class DateYmdDashTransformer implements TransformerInterface
{
    public function transform(string|array $source, TransformContext $context): TransformOutcome
    {
        if (is_array($source)) {
            return TransformOutcome::error('date_ymd_dash expects a single source column.');
        }
